...
function FeatureModule({ data, imageRight }: { data: ModuleData; imageRight: boolean }) {
  ...
  const textContent = (
    <div style={textStyle}>
      <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1a2e1f', margin: '0 0 12px' }}>
        {data.title}
      </h2>
      <p style={{ fontSize: '1rem', color: '#4b5563', lineHeight: 1.7, margin: '0 0 16px' }}>
        {data.subtitle}
      </p>
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
      ...
      <div style={{ textAlign: 'center', paddingTop: '16px', marginBottom: '8px' }}>
        <p style={{ fontSize: '1rem', fontWeight: 500, color: '#2ba357', margin: '0 0 8px' }}>
          为什么选择 mdocs
        </p>
