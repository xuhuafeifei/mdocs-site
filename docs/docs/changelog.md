...
### 手机端
- **目录树小三角**：点击只展开/收起子目录，不再顺带激活文件夹。此前手机端展开目录时会把整个目录抽屉关掉
- **Markdown 文章顶栏**：域选择和发布按钮恢复常驻单行；「可折叠操作栏」只对 HTML 文档生效（HTML 正文另有预览/编辑切换，顶栏更挤）

### 目录树
- **选中的文章常显图谱按钮**：不再只在鼠标悬停时出现，「查看图谱」「生成图谱」对选中项保持可见

## v1.3.5

### 知识图谱
- **工具栏重构**：左侧「标题 + 退出图谱」，右侧「层级控制 / 视图 / 配置 AI / 重新生成」
- **退出图谱**：红色实心按钮，桌面与手机都常显（此前只有手机端有个 `×`）
- **收纳**：展开一级 / 展开到二级 / 全部收起 合并为「层级控制」下拉；「显示其它关系」「显示 doc 节点」合并为「视图」下拉
- **标题**：过长时截断显示省略号，悬停可看全名
- **手机端**：右侧操作全部收进 `⋯` 三点菜单（竖排卡片，与文档页三点菜单一致）

### 手机端顶栏
- 文档顶栏（DocChrome）操作栏改为可折叠：第一行「目录 + 标题 + 展开 + ⋯」，域选择/发布等收进第二行，默认折叠

### 仓库
- 新增 **Android WebView 壳**（侧载 APK）与 **Tauri 2 桌面壳**（macOS + Windows）源码，均不参与 `pnpm build`，也不进 npm 包

## v1.3.4

### 登录 / 注册
- **去 Tab**：弹框不再用「注册 | 登录」双 Tab，**默认显示登录**
- 登录下方「没有账号？点击注册」进入注册流程；注册流程内保留「已有账号？点击登录」可随时切回

### 修复
- **弹框层级**：手机端展开侧边栏后，弹框不再被侧边栏遮挡（`z-index` 提升）

### 智能助手
- **项目信息**：助手现在能回答项目官网和源代码仓库地址

## v1.3.3

### UI 优化
- **编辑/预览开关**：改为 iOS 风格浅色分段控制器（`#F2F3F5` 背景 + 白色滑块）
- **顶栏按钮**：次要操作（收藏、评论等）统一灰色线性图标，hover 变深灰
- **侧边栏选中态**：去掉绿色边框，改为浅灰背景 + 左侧绿色指示条
- **布局**：侧边栏背景 `#FAFAFA`，主内容区 `#FFFFFF`，空间层次更清晰
- **树行间距**：增加行高和内边距，呼吸感提升
- **空白状态**：新增引导图标与文案

### 修复
- HTML 预览模式下右侧不再显示重复的编辑按钮（由分段控制器统一切换）

## v1.3.2

### HTML 文档
- **手机端顶栏**：Header 改为占据空间（docked），与面板贴合，不再悬浮在内容上

### 上传
- **不再限制文件类型**：任何扩展名均可上传，未知类型存为 `.bin`

## v1.3.1

### HTML 文档
- **手机端默认预览**：打开即预览，不再先显示源码
- 编辑 / 预览切换移入手机端三点菜单
...
### Agent
- **聊天 Mermaid**：助手回复里的 mermaid 围栏默认渲染成图，可切「图谱 / 代码」；只存源码，前端翻译，不存 SVG
- **图悬浮预览**：点击图谱进入悬浮层，可拖动平移、滚轮/按钮缩放
- **PC 全屏对话**：宽屏可切换助手面板全屏
- **图谱焦点 Ask**：打开知识图谱时，Ask 引用当前图谱缓存（只读），与文档焦点互斥

### 知识图谱与其它
- **单一焦点**：同一时刻只聚焦文档或图谱之一，切换时互清编辑器/图谱主区
- **文件类型政策**：按类型控制读取/覆写（禁止覆写图谱等特殊文件）；预留 HTML 类型
- **设置页表格**：收藏 / 我的文档表横向滚动包裹，窄屏更稳
...
### 知识图谱
- **异步生成队列**：点「生成图谱」入队后台执行，支持排队 / 进度面板 / 任务日志轮询
- **按篇进度**：目录与域构建按叶子文章上报进度（不再只有「整包 1/1」）
- **协作式让出**：长时间构建会按时间片 yield，避免占满并发槽
- **修复任务 ID 错位**：目录 `file_type` 为 `dir` 时误标成 `doc`，导致轮询 `not_found`、进度条闪退（后台其实在跑）
- **生成默认走缓存**：`force: false`，干净缓存可复用；脏数据才会重抽
- **队列文案**：下一位显示「即将开始」，不再出现「前面还有 0 个任务」
- **目录树图谱入口**：悬停操作改为 SVG 图标；长标题省略 + 悬停全名

### 体验与其它
- **手机阅读顶栏**：常用操作保留，更多收入菜单，减轻窄屏拥挤
- **CI**：GitHub Pages 部署相关 Action 升级到 v5
...
### 知识图谱 🧠
- **三级构建**：文章 / 目录 / 域，自下而上 AI 归纳概念与关系
- **两种节点**：文档节点 + 概念节点
- **四种关系**：包含（contains）、相关（related_to）、部分（part_of）、依赖（depends_on）
- **力导向图可视化**：可拖拽、缩放、悬停高亮；右侧详情面板展示节点信息
- **脏缓存机制**：正文改动超过阈值才标脏重算，小改不触发 AI
- **异步生命周期**：删/挪/发布后后台更新图谱，不阻塞主链路
- **入口**：目录树悬停按钮、域侧栏「知识图谱」按钮

### AI 智能助手 🤖
- **多供应商配置**：DeepSeek 预设 + 自定义供应商，一人可存多条配置
- **极简自定义**：只需填 baseUrl / modelId / apiKey 三个字段
- **协议自动识别**：URL 含 `anthropic` 自动切换 Anthropic 协议
- **apiKey 文件存储**：从数据库迁移到 `~/.mdocs/secrets/`，文件权限 0600
- **默认配置切换**：多配置时可设默认，一键切换
- **内嵌配置卡片**：助手面板直接填 Key，不用跳转设置页

### 其他
- **设置页 AI 配置全面翻新**：列表视图、创建/编辑/删除/设为默认
- 帮写工作台适配多供应商配置
...
- **手机助手全屏**：窄屏 Agent 面板铺满、FAB 不可拖；PC 浮层拖动不变
- **Ask 挂载上下文**：每轮对话把当前 `domainId` / 打开中的 `documentId` 写入 system（不预拉全文；读正文仍走 tool）
- **合并内联 UI**：冲突改为单栏红（我的）/ 绿（别人的）；接收=用别人的、拒绝=用我的；全部用我的 / 全部用别人的；未决议完只读，决议完再编辑发布
...
- **帮写编辑**：接收中与默认均可直接改「我的稿」；有提案时点「查看 diff」进入只读审阅
- **取消等待**：选择卡 / Skill 表单支持取消；有倒计时卡时关闭助手会确认是否强制退出
...
- **更快看到侧栏树**：启动时延后全文索引重建（默认约 15s，可用 `MDOCS_INDEX_INITIAL_DELAY_MS` 调整），减轻 HTML TTFB 被 CPU 拖慢
- **加载壳**：`index.html` 增加「mdocs 加载中…」占位，减少白屏感

## v0.8.17

- **AI 双模式定位明确**：智能助手（Ask）偏低交互——可答疑、搜文、结构操作，也能直接全文覆写；帮写（Coding）偏高交互——左聊右 Diff、按段接受、你拍板后才写回
- **智能助手：覆写选择卡**：有正文时请求写作，弹出三选一：直接覆写 / 打开帮写审阅 / 取消
- **智能助手：写后同步**：覆写成功后若当前打开该文档，自动触发拉取更新逻辑，刷新 headCommitId
- **帮写欢迎文案**：明确「精细改稿请用帮写」，不再说「不协助写作」
- **官网文档**：补充双模式 8 维度对比表，更新首页卖点

## v0.8.16

- **帮写（coding）**：编辑器顶栏 / 欢迎页入口进入全屏工作台；左 AI、右纯 Markdown；按段接受/拒绝提案后再写回（已有文进草稿，空白帮写可新建）
- **帮写上下文**：每轮请求携带进场快照与当前工作稿；助手可「读取帮写工作稿」后再「设置帮写正文」，避免盲改
- **帮写会话**：独立于上手助手；按文档隔离（A 文 / B 文各自历史）；空白帮写写回成文后会话绑到新文档
- **附件上传**：支持 zip 与常见音频；单文件上限 55MB
- **编辑器依赖**：`@fgbg/lobe-editor@1.0.0-fork.17`

## v0.8.15

- **智能助手读文**：新增 `get_document` 工具，按当前访客读权限读取文档正文（与 HTTP 鉴权一致，无额外特权）
- **助手时间线 UI**：对话中按顺序插入工具结果（搜文/域树表格、读文预览卡等），默认折叠；历史重开不回放工具 UI
- **代码块复制**：助手回复里的 Markdown 代码块支持一键复制
...
- **编辑器顶栏**：撤销/重做后新增「文档顶部插入空行」「文档末尾插入空行」
- **斜杠菜单**：增加 Meta2d、Markmap 快捷插入
- **发布后视口**：服务端正文与刚发布内容一致时不再整棵 `setDocument` 重挂，减轻 Mermaid 等块导致的跳动
...
- **发布后视口跳动**：发布成功后仍会拉取最新 meta / head 并清除本地草稿，但当服务端正文与刚发布内容一致时，不再对编辑器执行整棵 `setDocument` 重挂，避免 Mermaid 等块因 DOM 重建导致视觉窗口跳动
- **编辑器依赖**：升级 `@fgbg/lobe-editor@1.0.0-fork.14`（Mermaid 渲染结果缓存与加载占位，减轻重挂场景下的高度塌陷）
