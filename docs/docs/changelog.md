...
现行文档里，产品概念「域」已改称 **工作空间**。下面各版本仍按当时的叫法记录。

## v1.3.17

### 侧栏与目录
- **右键可新建目录**：文档树右键除「新建文档」外，恢复「新建目录」
- **侧栏工作入口**：左侧保留「新建文章」「新建目录」；欢迎页主按钮仍是「开始写作」

### 删除按钮
- 删除按钮改为红描边 + 红字，悬停再加深浅红底，默认状态下文字更清楚

### 知识图谱权限
- 读图 / 生成图谱须具备工作空间协作身份（`full`）；仅靠单篇邀请进入的成员不能访问图谱
- private 工作空间仅主人可访问图谱；接口返回明确错误文案

### 图谱构建
- 目录归纳进度文案改用目录显示名，不再误用 document id

## v1.3.16

### 顶栏状态
- **「编辑中 / 已发布」只跟未发布的本地草稿**：没有草稿就是已发布；草稿写入或删除后，顶栏同一轮改字。打开已有草稿时不再先闪一下「已发布」
- 打字后仍等约 1 秒落盘，才变成「编辑中」

### AI 帮写
- 桌面端「完成并写回」「取消」改用应用内确认框。系统确认框在桌面壳里不会弹出，所以之前点了没反应
- 完成时只问「是否回写」：确认写入本地草稿，不自动发布；返回留在帮写界面
...
### 我的文章
- **筛选区横向排布**：搜索框与域/创建者/分组筛选同一行，聚焦态改为细边框，不再整块挤成竖排
- **表格可读性**：标题主次分明，日期 `YYYY-MM-DD`，操作列「邀请」次要、「打开」主按钮
- **跳页**：页脚支持「跳至」指定页码（回车或点跳转），超出范围自动夹到首尾页

### 域管理
- **改类型**：行内分段控件（公开 / 受限 / 私有）+ 勾选确认 / 取消；不能下降的选项置灰，悬停才提示
- **操作列层级**：「成员…」主色按钮，重命名/改类型为文字按钮，删除为危险色；禁用态灰底可辨
- **内置域**：操作列改为「系统预设，不可修改」标签

### 智能助手
- **`query_my_documents`**：原 `list_my_documents` 改名；可按域/创建者筛选、分组，并用 `offset`/`limit` 翻页（与设置页「我的文章」对齐）
- 全文检索仍用 `search_documents`

## v1.3.12

### 域权限
- **只能升级，不能下降**：开放程度 `private < restricted < public`；更封闭返回 400
- 有文档的域仍可升级权限；重命名/删除在有文档时继续锁定
- Agent 工具 `set_domain_permission` 走同一规则

### 我的文章
- 支持按域、创建者筛选，以及按域/创建者分组（本页）
- 列表行增加创建者信息；筛选参数均可不传

### 附件下载
- 桌面端 / 手机端点击文件链接可正常下载（Web 拦截 + 服务端强制 attachment + 壳层处理新窗口）

### 智能助手
- `invite_document_user`：按文档邀请访客（read / edit）
- `query_my_documents` 的前身已支持筛选参数
