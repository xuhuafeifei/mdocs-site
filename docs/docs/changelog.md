...
### UI 优化
- **编辑/预览开关**：改为 iOS 风格浅色分段控制器（`#F2F3F5` 背景 + 白色滑块），取代之前的深蓝色滑动开关
...
### 上传
- **不再限制文件类型**：任何扩展名均可上传，未知类型存为 `.bin`，前端默认显示文件图标

### 修复
- HTML 预览模式下右侧不再显示重复的编辑按钮（由分段控制器统一切换）
- HTML 手机端默认预览模式，编辑/预览切换移入三点菜单
- HTML 手机端 Header 改为占据空间（docked），与面板贴合
