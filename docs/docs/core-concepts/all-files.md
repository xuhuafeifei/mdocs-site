...
|-------------|------|-----------------|-------------|
| `dir` | 目录 | 工作空间内相对路径（如 `folder/sub`） | 指向父目录的 `document_id` |
...
- `{domain_id}` 是工作空间的唯一标识，每个工作空间拥有独立的磁盘子目录
...
- 前端用其内容作为目录描述展示
- **v0.8.5+**：与普通 `.md` 文档相同，拥有 `headCommitId` 与发布版本链；编辑目录描述后可正常发布

## 设计意图

- SQLite 管理逻辑结构（谁是父目录、谁有权限）
- 文件系统管理实际数据（Markdown / JSON 内容）
- 工作空间之间在磁盘上完全隔离，不同工作空间可以拥有同名文件
