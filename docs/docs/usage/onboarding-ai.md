# 上手助手（AI）

mdocs 内置两种 AI 能力，共用设置页里的 DeepSeek 配置：

| 模式 | 入口 | 做什么 |
|------|------|--------|
| **上手助手（Ask）** | 左下角悬浮球 | 答疑 + 账号内结构操作（搜文、列树、读文、建空文档/文件夹等）。**不会**代写或改正文 |
| **帮写（Coding）** | 编辑器顶栏「帮写」/ 欢迎页「AI 帮写」 | 全屏工作台起草或改稿；右侧是纯 Markdown，按段接受后再写回 |

若你要用 Cursor / Claude 等**外部 Agent** 读写知识库、落开发契约，见 [Agent 开发闭环](./agent-dev-loop.md)。

## 入口（上手助手）

左下角有圆形 **上手助手** 悬浮按钮（不占用正文区）。点击后打开浮层 **「mdocs 智能助手」**，主编辑区仍保持打开，可边问边写。可**按住拖动**入口到任意位置（本地记住）；挪过之后会出现「重置位置」。

![上手助手入口与聊天浮层](./onboarding-ai/entry.png)

浮层内：

- 顶部：**+** 新建会话、历史列表、关闭
- 中间：对话与指引内容（可含步骤、表格、工具结果折叠块等）；回复中的代码块可一键复制
- 底部输入框：占位「把你的问题告诉我…」
- 页脚提示：内容由 AI 生成，仅供参考

## 帮写（Coding）

需要代写、润色或改某一篇正文时，用 **帮写**，不要用上手助手浮层。

1. 打开一篇文档（或欢迎页点「AI 帮写」从空白开始）
2. 点顶栏 **帮写**（带 DeepSeek 标志）进入全屏层：左侧对话，右侧 Markdown
3. 描述要写/改的内容；助手会先读当前工作稿与进场快照，再提交完整 Markdown 提案
4. 右侧按变更段 **接受 / 拒绝**；也可继续手改
5. **完成并写回**：已有文档写入本地草稿（需你再发布）；空白帮写则创建新文档

会话说明：

- 帮写会话与上手助手**分开存放**（`agent/coding-session/`）
- **按文档隔离**：文章 A 的历史看不到文章 B
- 左侧同样有 **+** / 历史；历史只回放对话文字，不恢复右侧稿
- 从空白帮写写回成文后，当前对话会绑到新文档，下次打开该文帮写可续聊

## 开始使用前：配置模型

1. 打开 **设置 → AI**（侧栏底部访客信息进入设置）
2. 选择模型：`deepseek-v4-flash` 或 `deepseek-v4-pro`
3. 填写你的 DeepSeek API Key 并保存

![设置 → AI：上手助手模型配置](./onboarding-ai/ai-settings.png)

未配置 Key 时，入口可用但无法发送消息，浮层 / 帮写会提示去设置页配置。

每人一份配置，仅本人可用；Key 在服务端按访客隔离存储，不会出现在别的访客界面上。配置页可查看配置名称、模型、脱敏后的 Key 与配置 ID；需要改时可点 **编辑**。

## 怎么问（上手助手）

可以直接问，例如：

- 如何发布文档？
- 草稿是什么？
- 如何创建域？
- 权限怎么设置？
- 帮我搜索 / 列出某域文档，并打开其中一篇说明要点

助手会按需阅读 [mdocs 用户手册](https://xuhuafeifei.github.io/mdocs-site/) 相关章节，也可在你有权阅读的范围内读取知识库文档。工具结果（搜索表、域树、读文预览等）会出现在对话时间线中，**默认折叠**，展开可查看；历史会话重开后通常只保留文字、不再回放这些工具块。答完后通常会附带一两个可继续追问的示例。

## 会话（上手助手）

- 同一访客的对话会保存在数据目录 `tenant/<访客ID>/agent/session/` 下，刷新或重开浮层仍可续聊。
- 浮层标题栏 **「+」**：新建空会话并切换过去。
- **历史图标**：按最近更新列出会话；点击即可切换并回放该会话（纯文本；工具 UI 不落盘）。
- 会话标题取自**第一条用户消息**（过长会截断），不是模型摘要。
- 删除、重命名、按天数自动清理尚未提供。

## 明确不会做（上手助手）

- 代写、润色、按选区改文档正文（请改用 **帮写**）
- 越权读取你无权查看的文档（与网页端同一套权限）
- 静默替你发布或改权限档（结构操作会走工具；写作类请求会拒绝并引导去帮写）

若你在上手助手里要求写作，助手会拒绝，并提示打开 **帮写**。
