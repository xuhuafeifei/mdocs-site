...
浮层内：
- 顶部：**+** 新建会话、历史列表、关闭
- 中间：问候语、快捷提问（如「如何发布文档？」「草稿是什么？」「如何创建工作空间？」）与对话内容
- 底部输入框：占位「把你的问题告诉我…」
- 页脚提示：内容由 AI 生成，仅供参考
...
2. 选择 **Deepseek** 标签页
3. 选择模型：`deepseek-v4-flash` 或 `deepseek-v4-pro`
4. 填写你的 DeepSeek API Key 并保存

![设置 → AI：智能助手模型配置](./onboarding-ai/ai-settings.png)

未配置 Key 时，入口可用但无法发送消息，浮层 / 帮写会提示去配置。

> 💡 **快捷方式**：直接点智能助手面板里的「配置」卡片，不用进设置页也能填 Key。

### 自定义供应商

如果你用其他模型服务商（OpenAI、Anthropic、自建网关等），可以用**自定义配置**：

1. 打开 **设置 → AI**，点「**+ 新建配置**」
2. 切换到 **自定义** 标签页
3. 填写三个必填项：
   - **API 地址** — 你的服务商 API 根地址（如 `https://api.openai.com/v1`）
   - **Model ID** — 模型名称（如 `gpt-4o`、`claude-3-5-sonnet-20241022`）
   - **API Key** — 你的密钥
4. （可选）高级选项里可以调：
   - Provider ID — 自定义标识，不填会自动从域名取
   - API 协议 — OpenAI 兼容 / Anthropic 兼容（URL 含 `anthropic` 会自动识别）
   - 配置备注名、Context Window
...
- API Key 以文件形式单独存储在 `~/.mdocs/secrets/agent-api-keys.json`，文件权限 0600（仅本人可读写）
- 数据库中不存 Key 明文
- 按访客隔离，你配置的 Key 只有你能用
- 不会出现在任何前端返回中（只返回「是否已配置」和脱敏尾号）

### 私人 Agent Skills

同一页下方还可管理 **Agent Skills**（私人提示词宏）：

- 名称对本账号唯一，仅英文、数字、下划线
- 简介可选；正文为 Agent 应遵守的步骤与约束
- 在智能助手 / 帮写输入区点 **Skill** 多选引用；本轮发送成功后选中标签会清空
- 也可让助手用工具新建 / 修改（表单卡 + 倒计时）、罗列或按名删除

落盘与会话历史只保存 skill **名称引用**，不会把正文全文写进聊天记录；请求时再按名称展开进模型上下文。
...
助手会按需阅读 [mdocs 用户手册](https://xuhuafeifei.github.io/mdocs-site/) 相关章节，也可在你有权阅读的范围内读取知识库文档。工具结果（搜索表、目录树、读文预览等）会出现在对话时间线中，**默认折叠**，展开可查看；历史会话重开后通常只保留文字、不再回放这些工具块。答完后通常会附带一两个可继续追问的示例。
