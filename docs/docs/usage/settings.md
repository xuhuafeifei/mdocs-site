...
设置页面是 mdocs 的集中配置中心：身份与密码、收藏与文章、工作空间与成员模板、智能助手、草稿同步等都在这里。
...
| [工作空间](./domain-members.md) | 创建 / 筛选 / 重命名 / 改类型 / 删除工作空间；受限工作空间成员 |
| [成员模板](./domain-members.md) | 可复用的访客名单 |
...
| 工作空间 / 成员模板 | [受限工作空间成员与名单模板](./domain-members.md) |
...
语言、自动编辑、自动同步等偏好多在浏览器本地；与身份绑定的数据（收藏、工作空间与成员、模板、CLI Token、密码哈希、恢复码哈希、AI 配置）在服务端。

### 为什么看不到某些操作？

- **工作空间**：重命名 / 删除等通常仅创建者可见
- **受限工作空间「成员」**：仅受限工作空间且你是创建者时才有
- **邀请成员**：仅自己创建的文档显示
